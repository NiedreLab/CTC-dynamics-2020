%% Create simulations with a changing mean
% Inputs
recreate_flag = 0; % Set to 1 to recreate simulations
...
if recreate_flag == 1
    ...
    % detections
    load('Simulations_ChangingMeanPoissons_detections.mat', 'detections', 'num_detections');
...
for i = 1:num_simulations
    ...
    if recreate_flag == 1
        ...
        num_cells_per_avg = round(scan_length_minutes*avgs*60/2);
        interarrivals = []; % Array that will hold the final detection rates
        for j = 1:2
            ...
            % Make sure the simulated scan is "length_of_time/2" minutes long
            % Add detection times if under the desired length of time
            while detection_times_tmp(end) < (scan_length_minutes/2)*60
            ...
            if detection_times_tmp(end) > (scan_length_minutes/2)*60
                detection_times_tmp = detection_times_tmp(detection_times_tmp < scan_length_minutes*60/2);
                times_between_detections = times_between_detections(1:length(detection_times_tmp));
            end
            fprintf('\t%d detections\n', length(times_between_detections));
            interarrivals = [interarrivals; times_between_detections];
        end
        
        % Create the final array of detection times
...
if save_data_flag == 1
    if recreate_flag == 1
        save('Simulations_ChangingMeanPoissons_detections', 'detections', 'num_detections', 'input_avgs');
    end
    save('Simulations_ChangingMeanPoissons_data', 'avg_CTCs_per_interv', 'interval_variance');
end
